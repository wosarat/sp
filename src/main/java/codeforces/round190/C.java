...
public class C {
    ...
    private void solve(){
        ...
        for(char c:route.toCharArray()){
            ...
            if(dx==0){
                ...
                if(getIy(My, nextY,dy)>0){
                    out.println(YES);
                    return;
                }
            }else{
                int ix=getIx(Mx, nextX,dx);
                if(ix<=0){
                    continue;
                }
                if(My==nextY+dy*ix){
    ...
    private static int getIy(int My, int nextY, int dy) {
        if((My-nextY)%dy!=0){
            return -1;
        }
        return (My-nextY)/dy;
    ...
    private static int getIx(int Mx, int nextX, int dx) {
        if((Mx-nextX)%dx!=0){
            return -1;
        }
        return (Mx-nextX)/dx;
     ...
     * @return
     */
    private static boolean isOk(int nextX, int nextY, int mx, int my) {
        return nextX==mx&&nextY==my;
    }
