...
public class C implements Runnable {
    ...
    private void solve(){
        int n = in.nextInt();
        Map<Fraction, Integer> counts=new HashMap<Fraction, Integer>();
        for(int i=0;i<n;i++){
            int a = in.nextInt();
            int b = in.nextInt();
            int c = in.nextInt();
            int p=a+b+c;
            Fraction r2=new Fraction((p-2*a)*(p-2*b)*(p-2*c), p);
            Integer count = counts.get(r2);
            if(count==null){
                count=0;
            }
            count++;
        ...
        out.println(Collections.max(counts.values()));
    }
